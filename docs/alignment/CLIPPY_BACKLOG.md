...
The current `cargo clippy --workspace --all-targets --all-features -- -D warnings` run surfaces the issues below. Clear each
category before we restore the full lint gate in CI.

## Runtime
_Status: open (no fixes landed yet)_
...
_Status: open (cleanup pending)_
...
## CLI Enum Ergonomics
_Status: open (design decision needed)_
...
- Record progress in `docs/alignment/HANDOFF.md` (Next Actions) and log per-session details under `docs/dev_logs/` per the
  protocol reminder.