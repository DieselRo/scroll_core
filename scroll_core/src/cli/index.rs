...
pub fn index_list(archive_dir: &Path) -> Result<(), String> {
    ...
    println!("{}", serde_yaml::to_string(&idx.archive_index).unwrap_or_default());
    Ok(())
}

pub fn index_add(archive_dir: &Path, file: &str) -> Result<(), String> {
    let index_path = archive_dir.join("scroll_index.yaml");
    let mut idx = read_index(&index_path).unwrap_or(ArchiveIndex { archive_index: serde_yaml::Value::Mapping(serde_yaml::Mapping::new()) });
    ...
    let mut ensure_list = |section: &str| -> Vec<serde_yaml::Value> {
        if let Some(root) = idx.archive_index.as_mapping() {
            if let Some(ai) = root.get(&serde_yaml::Value::from("archive_index")) {
                if let Some(map) = ai.as_mapping() {
                    if let Some(seq) = map.get(&serde_yaml::Value::from(section)).and_then(|v| v.as_sequence()) {
                        return seq.clone();
                    }
                }
            }
    ...
    let entry = serde_yaml::Value::Mapping({
        ...
        m.insert(serde_yaml::Value::from("file"), serde_yaml::Value::from(file));
        m
    });
    if !core.iter().any(|v| v.get(&serde_yaml::Value::from("file")).and_then(|f| f.as_str()) == Some(file)) {
        core.push(entry);
    }
    ...
    let mut ai_map = root.get(&serde_yaml::Value::from("archive_index")).and_then(|v| v.as_mapping()).cloned().unwrap_or_default();
    ai_map.insert(serde_yaml::Value::from("core_scrolls"), serde_yaml::Value::Sequence(core));
    root.insert(serde_yaml::Value::from("archive_index"), serde_yaml::Value::Mapping(ai_map));
...
pub fn index_remove(archive_dir: &Path, file: &str) -> Result<(), String> {
    ...
    let mut root = idx.archive_index.as_mapping().cloned().unwrap_or_default();
    let mut ai_map = root.get(&serde_yaml::Value::from("archive_index")).and_then(|v| v.as_mapping()).cloned().unwrap_or_default();
    if let Some(seq) = ai_map.get_mut(&serde_yaml::Value::from("core_scrolls")).and_then(|v| v.as_sequence_mut()) {
