    ...
    pub fn from_env() -> Self {
        ...
        for s in &cfg.sinks {
            match s.as_str() {
                ...
                #[cfg(feature = "notifier_slack")]
                "slack" => {
                    if let Some(sn) = SlackNotifier::from_env() { sinks.push(Box::new(sn)); }
                }
                _ => {}
            }
        }
        Self { config: cfg, sinks, rate: Mutex::new(RateLimiter::new()), flap: Mutex::new(FlapSuppressor::default()) }
    }

    pub fn dispatch(&self, mut event: NotificationEvent) -> Result<()> {
        if !self.config.enabled { return Ok(()); }
        if !should_notify(&event) { return Ok(()); }
        ...
        if matches!(event.kind, NotificationKind::StatusChanged) {
            ...
            self.flap.lock().unwrap().record_status_change(&event.thread_id);
        }

        // Rate limit per thread per kind
        if !self.rate.lock().unwrap().allow(&event, self.config.rate_limit_minutes) {
            return Ok(());
        }

        // Ensure a concise template
        if event.reason.is_none() { event.reason = Some(String::new()); }
...
pub fn get_hub() -> Arc<NotificationHub> {
    HUB.get_or_init(|| Arc::new(NotificationHub::from_env())).clone()
}
...
pub fn notify_overdue_thread(thread_id: &str, title: &str, scroll_path: &str, reason: Option<&str>) -> Result<()> {
