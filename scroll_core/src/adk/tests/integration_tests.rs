...
mod tests {
    ...
    impl BaseLlm for FunctionCallingLlm {
        ...
        async fn generate_content<'a>(
            ...
            if weather_request {
                ...
                return Ok(Box::pin(futures::stream::once(futures::future::ready(response))));
            }
        ...
        function_tool!(
            ...
            |args: HashMap<String, serde_json::Value>, _context| async move {
                let timezone = args.get("timezone")
                    .ok_or_else(|| AdkError::InvalidRequest("Missing timezone parameter".to_string()))?
                    .as_str()
                    .ok_or_else(|| AdkError::InvalidRequest("Timezone must be a string".to_string()))?;
    ...
    async fn test_full_agent_tool_integration() {
        ...
        let session = runner.session_service.get_session(
            "test-app",
            user_id,
            session_id,
            None,
        ...
        assert!(session.events.len() >= 1, "Expected at least one event in the session");
        ...
        assert!(has_function_call || has_function_response, 
    ...
    async fn test_conversation_flow() {
        ...
        assert_eq!(session.events.len(), 4, 
            "Expected 4 events, got {}", session.events.len());
