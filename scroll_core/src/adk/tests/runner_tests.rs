...
mod tests {
    ...
    async fn test_runner_close_session() {
        ...
        let mut session = runner.session_service.create_session(
            "test-app",
            user_id,
            None,
            Some(session_id),
        ...
        assert!(session.is_some());
        let session = session.unwrap();
        assert_eq!(session.status, crate::adk::sessions::session::SessionStatus::Closed);
    }
}