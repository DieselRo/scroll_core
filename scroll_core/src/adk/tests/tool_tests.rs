...
mod tests {
    ...
    async fn test_basic_tool() {
        ...
        // Execute the tool
        let args = HashMap::from([("input".to_string(), serde_json::json!("test input"))]);
        ...
        let tool = function_tool!(
            ...
            |args: HashMap<String, serde_json::Value>, _context| async move {
                let input = args
                    ...
                    .as_str()
                    .ok_or_else(|| {
                        AdkError::InvalidRequest("Input must be a string".to_string())
                    })?;
    ...
    async fn test_tool_error_handling() {
        ...
        let tool = function_tool!(
            ...
            |args: HashMap<String, serde_json::Value>, _context| async move {
                let should_error = args
                    .get("should_error")
                    .ok_or_else(|| {
                        AdkError::InvalidRequest("Missing should_error parameter".to_string())
                    })?
                    .as_bool()
                    .ok_or_else(|| {
                        AdkError::InvalidRequest("should_error must be a boolean".to_string())
                    })?;
        ...
        let args = HashMap::from([("should_error".to_string(), serde_json::json!(false))]);

        let result = tool.execute(args, tool_context.clone()).await.unwrap();
        assert_eq!(result["result"], "No error occurred");

        // Test error case
        let args = HashMap::from([("should_error".to_string(), serde_json::json!(true))]);
    ...
    impl crate::adk::agents::base_agent::BaseAgent for TestAgent {
        ...
        async fn run_async<'a>(
            ...
            Pin<Box<dyn futures::Stream<Item = crate::adk::events::event::Event> + Send + 'a>>,
            AdkError,
        > {
