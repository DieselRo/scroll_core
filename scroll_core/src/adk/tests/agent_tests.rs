...
mod tests {
    ...
    impl BaseAgent for SubAgent {
        ...
        async fn run_async<'a>(
            ...
            Ok(Box::pin(futures::stream::once(futures::future::ready(event))))
        }
    }
    ...
    async fn test_llm_agent_basic() {
        ...
        assert_eq!(content.parts[0].text.as_ref().unwrap(), "This is a mock response");
    }
    ...
    async fn test_agent_streaming() {
        ...
        assert_eq!(content1.parts[0].text.as_ref().unwrap(), "Streaming response ");
        assert_eq!(content2.parts[0].text.as_ref().unwrap(), "Streaming response complete");
    }
}