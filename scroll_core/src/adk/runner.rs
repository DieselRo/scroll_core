...
impl Runner {
    ...
    pub async fn run<'a>(
            ...
            .await?
        {
            ...
            None => {
                self.session_service
                    .create_session(&self.app_name, user_id, None, Some(session_id))
                    .await?
            }
        };

        // Create invocation context
        let mut invocation_context = self.new_invocation_context(session, new_message, run_config);
        ...
        if let Some(content) = &invocation_context.user_content {
            ...
            self.session_service
                .append_event(&mut session, event)
                .await?;
        ...
        let event_stream = self.agent.run_async(context_clone).await?.boxed();
            ...
            .filter_map(move |event| {
                ...
                async move {
                    ...
                    if !event.partial.unwrap_or(false) {
                        if let Ok(Some(mut session)) = session_service
    ...
    pub async fn close_session(&self, session: &mut Session) -> Result<(), AdkError> {
        ...
        if let Some(memory_service) = &self.memory_service {
            memory_service
                .add_session_to_memory(session.clone())
                .await?;
        }
...
pub mod in_memory {
    ...
    pub fn new_in_memory_runner(app_name: String, agent: Arc<dyn BaseAgent>) -> Runner {
