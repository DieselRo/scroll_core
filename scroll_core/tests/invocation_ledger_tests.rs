...
impl ConstructAI for AllowConstruct {
    fn reflect_on_scroll(&self, _ctx: &ConstructContext) -> ConstructResult {
        ConstructResult::Insight { text: "ok".into() }
    }
...
fn test_invocation_logs_to_db() {
    ...
    match result {
        ...
        | ConstructResult::Refusal { .. }
        | ConstructResult::ScrollDraft { .. }
        | ConstructResult::ModifiedScroll(_) => {}
    }
}
