...
impl ConstructAI for EchoConstruct {
    fn reflect_on_scroll(&self, ctx: &ConstructContext) -> ConstructResult {
        ConstructResult::Insight { text: format!("{}:{}", self.0, ctx.emotion_signature.tone) }
    }
...
fn aelren_uses_tone_and_fallback() {
    ...
    if let scroll_core::construct_ai::ConstructResult::Insight { text } = aelren.invoke_symbolically(&s, &reg) { 
        assert!(text.starts_with("mythscribe:"));
    } else { panic!("expected insight") }
}
